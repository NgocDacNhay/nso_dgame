                    ...
                    if (util.compare_Day(Date.from(Instant.now()), p.nj.newlogin)) {
                        ...
                        if (p.nj.get().ItemMounts[4] != null) {
                            for (int i = 0; i < p.nj.get().ItemMounts[4].option.size(); i++) {
                                if (p.nj.get().ItemMounts[4].option.get(i).id == 66 && p.nj.get().ItemMounts[4].option.get(i).param > 0) {
                        ...
                        p.session.sendMessageLog("Bạn nhận được 100 lượng từ Điểm danh hàng ngày");
...
//        }
    ...
    public void sendCharInfo(@Nullable final User p, @Nullable final User revc) {
        ...
        try {
            ...
            Item item10 = p.nj.get().ItemBody[1];//bienhinh
            if (item10 != null) {
                if (item10.id == 1021) {//dao
                    m.writer().writeShort(165);
                } else if (item10.id == 1020) {//cung
                    m.writer().writeShort(166);
                } else if (item10.id == 1017) {//kiem
                    m.writer().writeShort(167);
                } else if (item10.id == 1022) {//quat
                    m.writer().writeShort(168);
                } else if (item10.id == 1019) {//kunai
                    m.writer().writeShort(169);
                } else if (item10.id == 1018) {//tieu
                    m.writer().writeShort(170);
                } else {
                    m.writer().writeShort(-1);
                }
            } else {
...
//            this.sendXYPlayer(p);
    ...
    public void attackAMob(@Nullable final Body body, @Nullable Mob curMob, int dame) throws IOException {
        ...
        if (this.map.cave == null && curMob.isIsboss() && body.getLevel() - curMob.level > 20 && curMob.templates.id != 237) {
            dame = 0;
        }
        ...
        if ((curMob.templates.id != 0 || p.nj.getTaskId() == 40) && curMob.lvboss != 3 && !curMob.isIsboss()) {
            ...
            if ((util.nextInt(100) < 50 || curMob.templates.id == 69) &&
                ...
                final ItemMap item = LeaveItem(TaskHandle.itemDrop(_ninja, curMob), _ninja.x + util.nextInt(-100, 100), _ninja.y);
        ...
        if (curMob.isDie) {
            ...
            if (this.map.isLangCo()) {
                if (util.nextInt(0,150) < 1) {
                    p.upluongMessage(util.nextInt(1,2));
                }
            }
            ...
            if (curMob.templates.id == 237 && p.nj.getCurrentMapId() == 160) {//vithu
                for (User player : this._users) {
                    ...
                    if (this.map.area[zoneid].getNumplayers() < this.map.template.maxplayers) {
                        ...
                        player.endLoad(true);
                        server.manager.sendTB(player, "Hướng dẫn", "Để nhặt được trứng cần có dây Nenshi trong hành trang. Nhặt trứng sẽ mất 10s, mất dây Nenshi ngay lập tức và không thể di chuyển cũng như đánh người khác, khi bị đánh mất máu hoặc chết sẽ nhặt trứng thất bại. Các nhẫn giả lưu ý!!!");
                    } else {
        ...
        if (curMob.isDie && curMob.level > 1) {
            ...
            if (this.map.cave != null && (this.map.getXHD() < 9 || (this.map.getXHD() == 10 && this.map.cave.level < 1))) {
...
//            arid = new short[]{12, 12, 12, 12, 12, 12, 12, 22, 22, 17, 17, 17, 17, 12, 12, 12, 659, 38, 454, 455, 456, 486, 487, 488, 489, 841, 842, 843};
            ...
            if (util.nextInt(1,100) < 12) {// tỉ lệ ra đồ 
                switch (curMobMaxLv) {// Vật phẩm rớt quái
                    case 10:
                        arid = new short[] {12,12,12,1,2,3,4,5,12,154,155,134,135,164,165,144,145,124,125,189,184,179,174,114,94,104,99,109,119};//thêm đá yên vật phẩm trang bị
                        break;
                    case 20:
                        arid = new short[] {12,12,12,1,2,3,4,5,12,156,157,136,137,166,167,146,147,126,127,190,185,180,175,115,95,105,100,110,120};
                        break;
                    case 30:
                        arid = new short[] {12,12,12,3,4,5,12,158,159,138,139,168,169,148,149,128,129,191,186,181,176,116,96,106,101,111,121};
                        break;
                    case 40:
                        arid = new short[] {12,12,12,4,5,12,160,161,140,141,170,171,150,151,130,131,192,187,182,177,117,97,107,102,112,122};
                        break;
                    case 50:
                        arid = new short[] {12,12,12,1,2,3,4,5,12,162,163,142,143,172,173,152,153,132,133,193,188,183,178,118,98,108,103,113,123};
                        break;
                    case 60:
                        arid = new short[] {12,12,12,1,2,3,4,5,12,325,326,327,328,329,330,317,318,319,320,321,322,323,324,332,331,333,334,335,336};
                        break;
                    case 70:
...
//                ItemMap im = this.LeaveItem(aridboss[util.nextInt(0,1)], curMob.x + util.nextInt(-20,20), curMob.y);
//                if (im != null) {
//                    im.item.quantity = 1;
//                    im.master = master;
//                }
...
//                aridboss = new short[]{552,553,554,555,556,557,558,559,560,561,562,563}; // boss langco
            }
            if (aridboss.length > 0) {
                ItemMap im = this.LeaveItem(aridboss[util.nextInt(0,aridboss.length-1)], curMob.x + util.nextInt(-20,20), curMob.y);
                if (im != null) {
                    im.item.quantity = 1;
                    im.master = master;
                }
        ...
        if (map.isLdgtMap() &&
            ...
            if (server.manager.EVENT != 0 && Math.abs(curMob.level - body.getLevel()) <= 10 && util.nextInt(100) < 30) {
                ...
                if (eventItems[index] != -1 && eventItems[index] != 298 && eventItems[index] != 299 && eventItems[index] != 300 && eventItems[index] != 301) {//bỏ đồ rớt sk
                    val itemMap = this.LeaveItem(eventItems[index], p.nj.x + util.nextInt(-40,40), p.nj.y);
                    if (itemMap != null) {
            ...
            if (randomIndex > 0 && arid[randomIndex] != -1 &&
                ...
                if (arid[randomIndex] == 12) {
                    ...
                    if (im != null) {
                        int quantity = 1;
    //                    if (im.item.id == 12) {
    //                        quantity = 500 * ((curMob.level / 10) + 1);
    //                    }
...
// rớt vật phẩm boss
    private void updateBossItemDrop(@Nullable final Mob mob) {
        ...
        if (map.isLangCo()) {
            mob.templates.arrIdItem = new short[] {5,6,7,8,9,10,12,456,454,455,552,553,554,555,556,557,545,547,443,485,436,437,549,550,551};
...
//             for (int k = 0; k < this.getUsers().size(); ++k) {
//                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)99, 1, 500);
//            }
//        }
//         if (p.nj.name.equals("admin")) {
//             for (int k = 0; k < this.getUsers().size(); ++k) {
...
//                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 11, 20050000, 3);
        ...
        if (p.nj.get().ItemBody[31].id == 967) {
            switch (p.nj.get().nclass) {
                case 1: {
                    for (int k = 0; k < this.getUsers().size(); ++k) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)16, 1, 500);
                    }
                    break;
                }
                case 2: {
                    for (int k = 0; k < this.getUsers().size(); ++k) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)17, 1, 500);
                    }
                    break;
                }
                case 3: {
                    for (int k = 0; k < this.getUsers().size(); ++k) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)15, 1, 500);
                    }
                    break;
                }
                case 4: {
                    for (int k = 0; k < this.getUsers().size(); ++k) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)13, 1, 500);
                    }
                    break;
                }
                case 5: {
                    for (int k = 0; k < this.getUsers().size(); ++k) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)12, 1, 500);
                    }
                    break;
                }
                case 6: {
                    for (int k = 0; k < this.getUsers().size(); ++k) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)14, 1, 500);
                    }
                    break;
                }
            }
        ...
        if (p.nj.get().ItemBody[10].id == 834) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)88, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 835) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)89, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 836) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)90, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 837) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)91, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 838) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)92, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 839) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)93, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 840) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)94, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 841) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)95, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 842) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)96, 1, 500);
            }
        }
        if (p.nj.get().ItemBody[10].id == 843) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short)97, 1, 500);
            }
        }
