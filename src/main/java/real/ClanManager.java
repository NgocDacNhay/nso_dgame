...
public class ClanManager {
    ...
    public void clanUpLevel(final @Nullable User p) throws IOException {
        ...
        if (p.nj.clan.typeclan == 4 || p.nj.clan.typeclan == 3) {
            ...
            if (this.getLevel() > 99) {
                p.session.sendMessageLog("Gia tộc đã đạt cấp tối đa");
                return;
            }
    ...
    public void requestClanItem(final @Nullable User p) throws IOException {
        ...
        for (ClanThanThu thanThu : this.clanThanThus) {
            ...
            if (item.id == 596 || item.id == 601) {
                m.writer().writeInt((int) (item.expires - System.currentTimeMillis() + item.timeBuy));
            } else {
                m.writer().writeInt(-1);
            }
            ...
            if (item.id == 596 || item.id == 601) {
                m.writer().writeUTF("Thời gian nở:");
            } else {
                for (Option option : item.option) {
                    if (option.id == ST_QUAI_ID) {
                        m.writer().writeUTF("Sát thương quái: " + thanThu.getOption(0));
                    } else if (option.id == ST_NGUOI_ID) {
                        m.writer().writeUTF("Sát thương người: " + thanThu.getOption(1));
                    }
                }
...
//            Server.clanTerritoryManager.addClanTerritoryData(ninja.p.getClanTerritoryData());
        }
