...
public class Mob {
    ...
    public int mapThieuDot;
    public int zoneThieuDot;
    public int zoneBoss;
    public int tuanlocid;
    ...
    public void updateHP(final int num) {
        ...
        if (this.hp <= 0) {
            ...
            if (this.isboss) {
                if (this.templates.id == 223) {
                    Server.timeRefreshBossSkHe = System.currentTimeMillis() + 1200000L;
                } else if (this.templates.id != 198 && this.templates.id != 199 && this.templates.id != 200 && this.templates.id != 223) {
...
//    }
    ...
    public void update(final @NotNull Place place) {
        if (isThieuDot &&
            ...
            if (effId == null || System.currentTimeMillis() > effId.timeRemove || this.isDie || this.mapThieuDot != masterThieuDot.c.getMapid() || this.zoneThieuDot != masterThieuDot.c.getPlace().getId()) {
                ...
                this.updateHP(dame);
                if (Math.abs(level - masterThieuDot.getLevel()) <= 10) {
                    val expUp = (long) this.level * dame / 50;
