    ...
    private static void useSkillBuff(Body body, final int skilltemp) throws IOException {
        ...
        switch (skilltemp) {
            ...
            case 52: {
                ...
                p.setEffect(20, 0, body.getPramSkill(47) * 1000, param);
                if (body.party != null) {
                    for (User p2 : p.nj.getPlace().getUsers()) {
                        if (p2.nj.id != p.nj.id) {
                            final Ninja n = p2.nj;
                            if (n.party == body.party && Math.abs(body.x - n.x) <= temp.dx && Math.abs(body.y - n.y) <= temp.dy) {
                                n.p.setEffect(20, 0, p.nj.get().getPramSkill(47) * 1000, param - 1000);
                            }
                        }
                    }
                    break;
            ...
            case 72: {
                ...
                p.setEffect(13, 0, 60000 * p.nj.getPramSkill(68), p.nj.getPramSkill(71));
