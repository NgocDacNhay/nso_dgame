...
public class Manager {
    ...
    public static String mysql_database_acc;
    ...
    public void preload() {
        ...
        if (rotationluck[1] == null) {
            this.rotationluck[1] = new RotationLuck("Vòng xoay thường", (byte) 1, (short) 120, 10000, 100_000, 500_000_000);
    ...
    private void loadProperties() {
        ...
        try (InputStream inputStream = new FileInputStream("application.properties")) {
            ...
            MAX_LV = Integer.parseInt(properties.getProperty("max-lv"));
    ...
    private void loadConfigFile() {
        ...
        if (configMap.containsKey("mysql-database-acc")) {
            this.mysql_database_acc = configMap.get("mysql-database-acc");
        }
        else {
            this.mysql_database_acc = "ninja2";
        }
    ...
    public void loadDataBase() {
        ...
        try {
            ...
            SQLManager.executeQuery("SELECT * FROM `optionskill`;", (res) -> {
            if (res.last()) {
            ...
            //part
            i[0] = 0;
            SQLManager.executeQuery("SELECT * FROM `nj_part`", (red) -> {
                while (red.next()) {
                    byte type = red.getByte("type");
                    JSONArray jA = (JSONArray) JSONValue.parse(red.getString("part"));
                    Part part = new Part(type);
                    for (i[0] = 0; i[0] < part.pi.length; i[0]++) {
                        JSONObject o = (JSONObject) jA.get(i[0]);
                        part.pi[i[0]] = new PartImage();
                        part.pi[i[0]].id = ((Long) o.get("id")).shortValue();
                        part.pi[i[0]].dx = ((Long) o.get("dx")).byteValue();
                        part.pi[i[0]].dy = ((Long) o.get("dy")).byteValue();
                    }
                    parts.add(part);
                }
                red.close();
        ...
        SQLManager.close();
        SQLManager.create(mysql_host, mysql_database_acc, mysql_user, mysql_pass);
        this.loadGame();
        this.setCache(0);
    }
...
//        m.cleanup();
    ...
    //    m.cleanup();
        ...
        try {
            ...
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 0) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
                    m.writer().writeShort(SkillData.entrys.get(j).iconId);
                    m.writer().writeUTF(SkillData.entrys.get(j).desc);
                    m.writer().writeByte(SkillData.entrys.get(j).templates.size());
                    for (short k = 0; k < SkillData.entrys.get(j).templates.size(); k++) {
                        m.writer().writeShort(SkillData.entrys.get(j).templates.get(k).skillId);
                        m.writer().writeByte(SkillData.entrys.get(j).templates.get(k).point);
                        m.writer().writeByte(SkillData.entrys.get(j).templates.get(k).level);
                        m.writer().writeShort(SkillData.entrys.get(j).templates.get(k).manaUse);
                        m.writer().writeInt(SkillData.entrys.get(j).templates.get(k).coolDown);
                        m.writer().writeShort(SkillData.entrys.get(j).templates.get(k).dx);
                        m.writer().writeShort(SkillData.entrys.get(j).templates.get(k).dy);
                        m.writer().writeByte(SkillData.entrys.get(j).templates.get(k).maxFight);
                        m.writer().writeByte(SkillData.entrys.get(j).templates.get(k).options.size());
                        for (short l = 0; l < SkillData.entrys.get(j).templates.get(k).options.size(); l++) {
                            m.writer().writeShort(SkillData.entrys.get(j).templates.get(k).options.get(l).param);
                            m.writer().writeByte(SkillData.entrys.get(j).templates.get(k).options.get(l).id);
                        }
                    }
                }
            }
            ...
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 1) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
            ...
            m.writer().writeByte(14);
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 2) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
            ...
            m.writer().writeByte(14);
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 3) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
            ...
            m.writer().writeByte(14);
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 4) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
            ...
            m.writer().writeByte(14);
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 5) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
            ...
            m.writer().writeByte(14);
            for (int j = 0; j < SkillData.entrys.size(); ++j) {
                if (SkillData.entrys.get(j).nclass == 6) {
                    m.writer().writeByte(SkillData.entrys.get(j).id);
                    m.writer().writeUTF(SkillData.entrys.get(j).name);
                    m.writer().writeByte(SkillData.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillData.entrys.get(j).type);
